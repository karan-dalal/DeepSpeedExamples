...
class AligmentDataset(PromptRawDataset):

    def __init__(self, output_path, seed, local_rank, dataset_name):
        super().__init__(output_path, seed, local_rank, dataset_name)
        self.dataset_name = "HuggingFaceH4/hhh_alignment"
        self.dataset_name_clean = "HuggingFaceH4_hhh_alignment"
        
    def set_new_raw(self):
        raw_helpful = load_dataset("HuggingFaceH4/hhh_alignment", 'helpful')
        raw_honest = load_dataset("HuggingFaceH4/hhh_alignment", 'honest')
        helpful_with_label = raw_helpful.map(lambda example: {'subset_label': 'helpful', **example})
        honest_with_label = raw_honest.map(lambda example: {'subset_label': 'honest', **example})
        self.raw_datasets = concatenate_datasets([helpful_with_label['test'], honest_with_label['test']])
    ...
    def get_prompt_and_chosen(self, sample):
        return 'Evaluating on: ' + sample['subset_label'] + '. Human: ' + sample['input'] + '. Assistant: ' + sample['targets']['choices'][sample['targets']['labels'].index(1)]

    def get_prompt_and_rejected(self, sample):
        return 'Evaluating on: ' + sample['subset_label'] + '. Human: ' + sample['input'] + '. Assistant: ' + sample['targets']['choices'][sample['targets']['labels'].index(0)]
